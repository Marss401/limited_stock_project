...
async function createReservation(//create a function that handles the reservations of the item with the userId, the productId and the quantity
...
//CHECKOUT FUNCTION
async function completeCheckout(reservationId) {
    return database_1.default.$transaction(async (tx) => {
        ...
        await tx.inventoryLog.create({
            data: {
                productId: reservation.productId,
                action: client_1.InventoryAction.PURCHASED,
...
async function expireReservations() {
    ...
    const expired = await database_1.default.reservation.findMany({
        where: {
            status: client_1.ReservationStatus.ACTIVE,
